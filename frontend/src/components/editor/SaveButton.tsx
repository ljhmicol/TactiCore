...
/**
 * 저장 버튼. 서버 미기동 시 비활성화 + 안내 문구를 보여준다 (FR-08 폴백,
 * 5단계 5-2-2). 저장 실패 시 서버 헬스를 재확인한다 (3단계 §2.1).
 */
export function SaveButton({ analysis }: { analysis: Analysis }) {
  ...
  const handleSave = async () => {
    setErrors(null)
    try {
      const saved = await saveMutation.mutateAsync(analysis)
  ...
  const disabled = !isServerUp || isChecking || saveMutation.isPending
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      <Button
        ...
        onClick={handleSave}
        disabled={disabled}
        title={!isServerUp ? '백엔드 서버가 꺼져 있어 저장할 수 없습니다' : undefined}
      >
